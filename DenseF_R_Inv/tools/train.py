...
def main():
    ...
    print("first Check Point reeched, we are just before the FIrst load of the NetWork")
    print('-------------------------------------------------------------------------------  \n ')
    estimator = PoseNet(num_points = opt.num_points, num_obj = opt.num_objects)
    estimator.cuda()

    # estimator = PoseNetPoseNetTranslation(num_points = opt.num_points, num_obj = opt.num_objects)
    # estimator.cuda()
    ...
    print(f'num point mesh size is : {opt.num_points_mesh} \n ')

    print('>>>>>>>>----------Dataset loaded!---------<<<<<<<<\nlength of the training set: {0}\nlength of the testing set: {1}\nnumber of sample points on mesh: {2}\nsymmetry object list: {3}'.format(len(dataset), len(test_dataset), opt.num_points_mesh, opt.sym_list))

    print('We hit Cp 2, we just loaded the dataset and skip the resume option hyyyyyaaaa ')
    print('--------------------------------------------------------------------------------- \n ')
    ...
    print('We hit CP 3, loss has been loaded for a first time, now its time to init the epoch ')
    print('-------------------------------------------------------------------------------------- \n ')
    ...
    for epoch in range(opt.start_epoch, opt.nepoch):
        ...
        for rep in range(opt.repeat_epoch):
            for i, data in enumerate(dataloader, 0):
                ...
                print('So its the end, we hit the savin time, time to get outahere')
                print('-----------------------------------------------------------  \n ')
                exit(0)
